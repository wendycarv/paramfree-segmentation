...
def display_data(joint_data, tf_data, gripper_data):
...
def plot_data(fname):
    ...
    print(fname)
    joint_data, tf_data, gripper_data = read_data(fname)
    fname = fname[:-3]
    #display_data(joint_data, tf_data, wrench_data, gripper_data)
    display_data(joint_data, tf_data, gripper_data)
    plot_joint_data(joint_data)
    plt.savefig(fname + '_joint.png')
    plot_tf_data(tf_data)
    plt.savefig(fname + '_tf.png')
    #plot_wrench_data(wrench_data)
    plot_gripper_data(gripper_data)
    plt.show()
    #plt.savefig(fname + '_gripper.png')
    print(fname)
    return

def main():
    path = "/Users/wendy/Desktop/school/uml/robotics/fetch_table_demos/h5 files/*.*"
    #filelist = list(os.listdir(path))
    filelist = glob.glob(path)
    filelist.sort()

    filename = '/Users/wendy/Desktop/school/uml/robotics/fetch_table_demos/h5 files/fetch_recorded_demo_1730997956.h5'#raw_input('Enter the filename of the .h5 demo: ')
    plot_data(filename)
